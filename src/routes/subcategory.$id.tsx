import { useQuery } from '@tanstack/react-query';
import { createFileRoute, useParams } from '@tanstack/react-router';
import { useNavigate } from '@tanstack/react-router'; 
import { Search } from 'lucide-react';
import { useState } from 'react';
interface SubCategory {
  id: number;
  foodSubCategoryId: number;
  foodSubCategoryName: string;
  foodCategoryId: number;
  foodCategoryName: string;
  leadFoodName: string;
  fullFoodName: string;
  aspartameWarning: boolean;
  isLowInProtien: boolean;
  foodExchangeId: number;
  foodRecipes: [];
  foodReferences: [];
  name: string;
  foodCategoryImagepath: string;
}

async function fetchSubCategory(pageNo: number, pageSize: number, id: number) {
  const response = await fetch(
    `https://api.pkubite.com/api/FoodSubCategory/GetAll?foodCategoryId=${id}&pageNo=${pageNo}&pageSize=${pageSize}`
  );
  if (!response.ok) {
    throw new Error('Network response was not ok');
  }
  const data = await response.json();
  return data.result.data;
}

export const Route = createFileRoute('/subcategory/$id')({
  component: RouteComponent,
});

function RouteComponent() {
  const { id } = useParams({ strict: false }); 
  const navigate=useNavigate()
  const [searchQuery,setSearchQuery]= useState('')
  const pageNo = 1;
  const pageSize = 10;
  
  const { isLoading, isError, data, error } = useQuery({
    queryKey: ['subcategory', id],
    queryFn: () => fetchSubCategory(pageNo, pageSize, Number(id)),
    
  });

  if (isLoading) {
    return <span>Loading...</span>;
  }

  if (isError) {
    return <span>Error: {error?.message}</span>;
  }
  const handleCardClick = (foodSubCategoryId: number) => {
    navigate({ to: `/subcategory/${id}/food/${foodSubCategoryId}` });
  };
  
  const filteredData = data?.filter((food: SubCategory) =>
    food.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const onSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearchQuery(e.target.value);
  };

  return (
    <div className="container mx-auto max-w-7xl bg-white rounded-lg p-4">
      <div className="p-4 flex items-center relative">
        <input
          type="text"
          placeholder="Search by category name"
          value={searchQuery}
          onChange={onSearch}
          className="p-2 border rounded-md w-full pr-10"
        />
        <Search className="absolute right-4" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4">
        {/* {data?.length > 0 ? (
          data.map((food: SubCategory) => (
            <div
              key={food.id}
              className="bg-gray-100 shadow-md flex flex-col items-center rounded-lg p-4 cursor-pointer"
              onClick={() => handleCardClick(food.id)} 
            >
              <div className="mt-2">
                <h3 className="font-semibold text-lg text-center">{food.name}</h3>
                <h3 className="font-semibold text-lg text-center">{food.foodCategoryName}</h3>
              </div>
            </div>
          ))
        ) : (
          <div className="col-span-full text-center">No Records found.</div>
        )}
      </div> */}
        {filteredData?.length > 0 ? (
          filteredData.map((food: SubCategory) => (
            <div
              key={food.id}
              className="bg-gray-100 shadow-md flex flex-col items-center rounded-lg p-4 cursor-pointer"
              onClick={() => handleCardClick(food.id)} 
            >
              
              <div className="mt-2">
                <h3 className="font-semibold text-lg text-center">{food.name}</h3>
                <h3 className="font-semibold text-lg text-center">{food.foodCategoryName}</h3>
              </div>
            </div>
          ))
        ) : (
          <div className="col-span-full text-center">No Records found.</div>
        )}
      </div>
    </div>
  );
}
